...
void ReadFile(const char* filename, map<string, Color>& colors)
{
	...
	while (getline(file, line)) {
		for (unsigned int i = 0; i < line.size(); i++) {
			...
			if (line.at(i) == ' ') {
				string colorname = line.substr(0, i);

				Color colorint;
				colorint.SetValue(stoi(line.substr(i + 1, line.size())));

				const char* cn = colorname.c_str();
				colorint.SetName(cn);
				colors[colorname] = colorint;
				//cout << i << ". " << colorname << " added!" << endl;
			}
		}
